...
%=================================================================
% Abstract
\abstract{Manual compliance auditing in African regulatory contexts consumes up to 40\% of IT
security budgets annually, yet existing approaches verify control \textit{presence} rather than
\textit{effectiveness}—leaving institutions vulnerable to adversarial evasion. This paper
presents an AI-augmented compliance auditor for Rwanda's National Cyber Security Authority
(NCSA) Minimum Cybersecurity Standards, combining multi-label XGBoost classification, adversarial
control validation, and Model Context Protocol (MCP)-integrated LLM analysis. We formalize
compliance effectiveness as $C(k) = \alpha D_k + \beta V_k + \gamma R_k$ (detection rate,
coverage, evasion resistance) and evaluate across 143 NCSA controls and 200{,}000 training
events. Key findings: XGBoost achieves 99.88\% F1 after 5\% domain fine-tuning but collapses
to 7.98\% zero-shot—a 92-point generalization gap bridged by the hybrid LLM path; adversarial
validation exposes effectiveness deficits invisible to checkbox auditing (SI-3: 20\% detection
rate; SI-10: 32\% XSS bypass); GPT-4o-mini achieves 93.5\% zero-shot accuracy across four log
types ($n$=200), while Llama-3.2-3B on CPU-only hardware achieves 84.0\%, validating
on-premise deployment for resource-constrained institutions. The system runs at 2.0 CPU cores,
\$50/month, producing audit reports in 0.77\,s—making effectiveness-based compliance auditing
accessible to African SMEs for the first time.}
...
out-of-distribution logs) resolves this gap---the LLM path achieves 83.3\% zero-shot
accuracy across three structurally distinct log types where XGBoost fails entirely.
...
engineered for Rwanda's NCSA 2023 framework---to our knowledge the first openly documented
such system for an African national regulatory context---with empirical validation on
...
Green \& Brown~\cite{bert_audit} & BERT (GPU) & No & No & No & N/A \\
\midrule
...
Our system occupies a distinct position: the only approach combining ML-based multi-label
evidence routing, adversarial effectiveness validation, African regulatory specificity, and
...
Windows Security Events & 50 & \textbf{100.0\%} & \textbf{100.0\%} & 90.4\% \\
\midrule
...
attributable to the addition of Windows Security Event logs (100\% accuracy on both models)
...
Events (100\%)---while Llama-3.2-3B on CPU-only hardware achieves 84.0\%, validating
...
(6) total resource footprint: 2.0 CPU cores, 2.66\,GB RAM, \$50/month.
