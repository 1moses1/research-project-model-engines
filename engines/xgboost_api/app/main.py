...
if not MODEL_DIR.exists():
    # Try new 196-control model first (look relative to project root)
    new_model = Path("../../models/xgboost_196controls").resolve()
    old_model = Path("../../models/compliance_auditor_final").resolve()

    if new_model.exists():
        MODEL_DIR = new_model
        print(f"ℹ️  Using NEW 196-control model from: {MODEL_DIR}")
    elif old_model.exists():
        MODEL_DIR = old_model
        print(f"ℹ️  Using OLD 50-control model from: {MODEL_DIR}")
    else:
        MODEL_DIR = Path("models/xgboost_196controls")  # Will fail, but with clear message
